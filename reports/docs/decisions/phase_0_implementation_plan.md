...
| D-0.5 | Great Expectations version & expectation-suite scope   | ✅ **D-0.5a: Option B (Verified GX 1.21 Core)**<br>✅ **D-0.5b: Option A (Approved)** | GX 1.21 Core Expectations API;<br>Minimal schema/dtype/null/range coverage |
...
**Requires your approval:** Yes  
**Decision Status:** ✅ **Approved — D-0.5a: Option B (GX 1.21 Core API, verified at implementation) & D-0.5b: Option A (Minimal, real coverage)**

**Sub-decision D-0.5a — API generation.**  
**Status:** ✅ **Approved (Confirmed at Implementation) — Option B**

| Option                                                            | Trade-offs                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Option A<br>The older, widely-documented Expectation Suite / Validator API | More Stack Overflow answers and tutorials historically, but legacy/deprecated in GX 1.x. Introduces deprecation warnings and architectural misalignment with installed `great-expectations 1.21.0`.                                                                                                                                                                                           |
| **[APPROVED - VERIFIED] Option B**<br>The newer Fluent/Core API (GX 1.x)   | Official supported API in GX 1.21+. Provides strongly-typed expectation classes (`great_expectations.expectations.core`), native JSON serialization in `gx/expectations/`, and clean integration with DataContext. Verification at Stage 4 confirmed GX 1.21.0 is installed, making Option B the stable, warning-free choice.                                                                |

**Recommendation:** Option B. Verified during Stage 4 implementation that GX 1.21.0 is the active release, where the Core Expectations API (`great_expectations.expectations.core`) is the stable production interface. Standardized in ADR-013.
...
_Completed at Phase 0 close (2026-09-21). Each row was verified against actual files before being recorded — no row accepted a "matches as designed" finding without a concrete artifact reference._
...
| ----- | ------------------------ | ----------------------- | ------- | -------- | ----------- | ------ |
| Dataset matches D-0.1 | Kaggle Company Bankruptcy Prediction, version-pinned | `data/raw/data.csv.dvc` — MD5 `da9cda1b8f7cb99d03fbb65b86c15b0f`; SHA-256 `67BF2E7C...FA14D8` pinned in ADR-011; 6,819 × 96 verified | Matches as designed. Both checksums pinned and referenced from ADR-011; DVC lock commits the MD5 deterministically. | None | — | ✅ Resolved |
| Evidence-bundle schema matches D-0.6 scope | Pre-v0 draft, `Optional` placeholders for Tiers 1–3 fields | `src/schemas/evidence_bundle.py` — `schema_version = "pre-v0-draft"` literal; `pd`, `pd_band`, `credit_rating`, `financial_ratios`, `persona_verdicts` all typed `Optional[...] = None`; `company_id` and `raw_features` required; `ConfigDict(extra="forbid", frozen=True)` enforced | Matches as designed. Pre-v0 skeleton strictly follows D-0.6: required fields for Phase 0 inputs, typed Optional stubs for all Tier 1–3 outputs, no v0 claim made. | None | — | ✅ Resolved |
| GX suite matches D-0.5b scope | Schema/dtype/null/range/uniqueness only — no drift checks | `src/pipelines/data_contracts.py` — expects 96 columns × 6000–7500 rows; `Bankrupt?` in `{0,1}`, 0% nulls; 11 core financial-ratio range checks `[0.0, 1.0]`; compound-column uniqueness; no statistical distribution or drift expectations present | Matches as designed. Scope is exactly minimal real coverage per D-0.5b / ADR-013; no drift or anomaly expectations in suite. | None | — | ✅ Resolved |
| CI matches D-0.7 scope | Lint + type-check + fixture-based unit test only | `.github/workflows/ci.yml` — jobs: `lint` (ruff), `typecheck` (pyright), `test` (pytest full suite incl. adversarial fixture tests), `docker-build` (no push). Full `dvc repro` absent per D-0.7 deferral. | Docker build job was not triggered on GitHub Actions at Stage 2 close because no push reached `main` after the job was added. Remediation: dedicated trigger commit `a47709a` (`ci: trigger workflow to verify docker job runs on Actions`) pushed to `main` on 2026-09-24 23:10 -03:00 (SHA `a47709a77f3c7faf7ccba32f456e3035b5ce26ca`). Actions run initiated; result was not visually confirmed at write time (GitHub Actions UI rate-limited). Confirm green on the Actions run page at https://github.com/SebastianGarrido2790/ACRAS/actions and update this cell if the run shows any failure. | Low | Trigger commit pushed to `main`; Actions run initiated 2026-09-24. Pending visual confirmation of green `docker-build` job on GitHub Actions. | ⏳ Pending confirmation |
| DVC remote matches D-0.3 | Local filesystem remote outside repo (Option A revised) | `.dvc/config` — `[core] remote = local_remote`; `[remote "local_remote"] url = ../acras_dvc_remote` (directory outside repo root). `dvc status` clean, `data/raw/data.csv.dvc` committed. | Matches as designed. Option A revised: local filesystem remote at `../acras_dvc_remote`, outside the repository working tree, satisfying DVC's reproducibility contract. S3 option (Option C) superseded per ADR-012. | None | — | ✅ Resolved |
| Repo structure matches D-0.0 | `pipelines/training/` vs `tier1_ml/` boundary respected | `src/pipelines/training/` contains only `stub_train.py` (Phase 0 no-op proof) and `__init__.py`; `src/tier1_ml/` contains only `__init__.py` (serving boundary placeholder). No training logic in `tier1_ml/`; no serving code in `pipelines/training/`. | Matches as designed. The ADR-010 boundary is clean at Phase 0: training pipeline stub in `pipelines/training/`; serving module in `tier1_ml/` is an empty placeholder with boundary comment. | None | — | ✅ Resolved |
| Exit criterion genuinely demonstrated | Corrupted fixture _actually_ fails the gate — the test itself must be shown to fail if the gate is removed, not just shown to pass | `tests/integration/test_gate_adversarial.py` — three tests: (a) valid data → gate passes → `stub_train` exits 0; (b) corrupted fixture (`tests/fixtures/corrupted_data.csv`, three deliberate violations) → gate raises `DataValidationError` → `stub_train` never reached; (c) falsification: monkeypatches `validate_dataset` to always return `True`, confirms corrupted fixture then reaches training — proving test (b) is gate-sensitive, not vacuously green. All three pass on final Stage 9 re-run (15/15 tests, 2026-09-21). | Matches as designed with genuine falsification. Gate is behaviorally verified, not just structurally. | None | — | ✅ Resolved |

All seven rows resolved. No unresolved severities. Phase 0 is signed off; Phase 1 may begin.
