...
| CI matches D-0.7 scope | Lint + type-check + fixture-based unit test only | `.github/workflows/ci.yml` — jobs: `lint` (ruff), `typecheck` (pyright), `test` (pytest full suite incl. adversarial fixture tests), `docker-build` (no push). Full `dvc repro` absent per D-0.7 deferral. Docker job runs `docker build` but was not triggered on GitHub Actions (documented Stage 2 exception). | Matches as designed with one documented exception: Docker CI job not triggered on GitHub Actions due to absence of a dummy change after the job was added. Local `docker build` verified instead. Exception logged at Stage 2 and carried forward; no blocking impact on gate. | Low (documented) | Accepted as Stage 2 exception; no remediation needed before Phase 1. | ✅ Resolved |
